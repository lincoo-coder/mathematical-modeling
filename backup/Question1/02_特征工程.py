...
def load_data():
    log("正在加载数据...")
    host_city_data = pd.read_csv("./pre_data/主办城市数据_编码结果.csv")
    medal_data = pd.read_csv("./pre_data/奖牌数据_编码结果.csv")
    athlete_data = pd.read_csv("./运动员统计特征.csv", encoding='gbk')
    sports_data = pd.read_csv("./pre_data/项目数据_编码结果.csv")
    log("数据加载完成")
